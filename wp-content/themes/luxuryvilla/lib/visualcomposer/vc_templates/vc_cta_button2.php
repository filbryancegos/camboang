...
?>
<div<?php echo wp_kses_post($inline_css); ?> class="<?php echo esc_attr(trim($css_class)); ?>">
    <?php if ($link!='' && $position!='bottom') echo do_shortcode('[vc_button2 add_icon="'.$add_icon.'" icon="'.$icon.'" icon_align="'.$icon_align.'" link="'.$link.'" title="'.$title.'" color="'.$color.'" text_color="'.$text_color.'" icon="'.$icon.'" size="'.$size.'" el_class="vc_cta_btn"]'); ?>
<?php if ($h2!=''): ?>
    <hgroup>
        <?php if ($h2!=''): ?><h2 <?php echo wp_kses_post($inline_h_css); ?> class="wpb_heading"><?php echo esc_html($h2); ?></h2><?php endif; ?>
    </hgroup>
<?php endif; ?>
    <p <?php echo wp_kses_post($inline_h_css); ?>><?php echo wp_kses_post($content); ?></p>
    <?php if ($link!='' && $position=='bottom') echo do_shortcode('[vc_button2 add_icon="'.$add_icon.'" icon="'.$icon.'" icon_align="'.$icon_align.'" link="'.$link.'" title="'.$title.'" color="'.$color.'" text_color="'.$text_color.'" icon="'.$icon.'" size="'.$size.'" el_class="vc_cta_btn"]'); ?>
</div>
