...
<div class="<?php echo apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $css_class, $this->settings['base']) ?>">
    <div class="wpb_wrapper">
        ...
        <div id="<?php echo esc_attr($carousel_id); ?>" 
            class="owl-carousel <?php echo esc_attr($has_magnific); ?>"
            data-slides-per-view = "<?php echo esc_attr($slides_per_view); ?>"
            data-single-item = "<?php echo esc_attr($slides_per_view === '1' ? 'true' : 'false'); ?>"
            data-transition-slide = "<?php echo esc_attr($transition_style); ?>"
            data-navigation-owl = "false"
            data-lazyload = "true"
            data-pagination-owl = "<?php echo esc_attr($hide_pagination_control !== 'yes' ? 'true' : 'false'); ?>"
            data-autoplay = "<?php echo esc_attr($autoplay === 'yes' ? 'true' : 'false'); ?>"
            data-rewind = "<?php echo esc_attr($wrap === 'yes' ? 'true' : 'false'); ?>"
            ...
            <!-- Wrapper for slides -->
                    ...
                    if ($attach_id > 0) {
                        ...
                        if ($img_size !== 'fullsize') {
                            $thumbnail = '<img class="wp-post-image lazyOwl '.$img_style_class.'" data-src="'.gg_aq_resize( $attach_id, $customsize_width, $customsize_height, true, true ).'" alt="'.$alt_text.'" />';
                        } else {
                            $thumbnail = '<img class="wp-post-image lazyOwl '.$img_style_class.'" data-src="'.$attachment_url.'" />';          
                        }
                    }

                    ?>
                    <div class="item <?php echo esc_attr($slides_per_view === '1' ? 'single-image' : ''); ?>">
    ...
    </div><?php echo wp_kses_post($this->endBlockComment('.wpb_wrapper')); ?>
</div><?php echo wp_kses_post($this->endBlockComment('.wpb_images_carousel')); ?>